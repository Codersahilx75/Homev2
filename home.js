...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function toggleNavMenu() {
    ...
    menu.classList.toggle('transition-all');  // Apply transition to smooth menu appearance/disappearance
}
...
document.addEventListener('DOMContentLoaded', function () {
  const carousel = document.querySelector('.third-cards-wrapper');
  ...
  let totalSlides = cards.length - 3; // Total slides based on visible cards (3)

  // Create pagination dots dynamically based on the number of slides
  for (let i = 0; i <= totalSlides; i++) {
      const dot = document.createElement('div');
      dot.classList.add('dot', 'w-3', 'h-3', 'bg-gray-400', 'rounded-full', 'cursor-pointer');
      if (i === 0) dot.classList.add('bg-blue-500'); // Active dot
  ...
  function updateDots() {
      const activeSlide = Math.round(scrollPosition / cardWidth);
      dots.forEach((dot, index) => {
          dot.classList.toggle('bg-red-500', index === activeSlide);
          dot.classList.toggle('bg-gray-400', index !== activeSlide);
      });
  }

  // Scroll to the next slide
  nextBtn.addEventListener('click', () => {
      const maxScroll = cardWidth * (cards.length - 3); // Max scroll for 3 visible items
  ...
  // Dot click event for direct navigation
  dots.forEach((dot, index) => {
